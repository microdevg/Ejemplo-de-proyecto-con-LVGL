...
bool button_read_cb(lv_indev_t *indev, lv_indev_data_t *data) {
    bool current_state = HAL_GPIO_ReadPin(BLUE_BUTTON_GPIO_Port, BLUE_BUTTON_Pin);
    
    data->state = (current_state == 0) ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED;
    return false;  // No hace falta indicar "hay nuevo dato", solo reportar el estado
}
